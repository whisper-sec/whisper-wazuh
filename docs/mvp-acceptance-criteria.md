...
| config knobs `api_url`, `dedup_ttl`, `dedup_scope`, `deadline`, `max_iocs` resolve `<options>` JSON (argv[5]) → env (`WHISPER_API_URL`/`WHISPER_DEDUP_TTL`/`WHISPER_DEDUP_SCOPE`/`WHISPER_DEADLINE`/`WHISPER_MAX_IOCS`) → default (mapping §2.3) | required implementation knobs — TC-10/TC-13 are untestable without them (§5 item 6) |
...
| TC-13 | Whisper API unreachable | Set `<options>{"api_url":"https://localhost:1/"}` (§1.1 knob), restart, inject | The whole invocation is bounded by the per-alert `deadline` (default 20 s): every request's timeout and retry backoff shrink to what is left, a SIGALRM hard ceiling at deadline + 1 s backs that up if a socket operation itself stalls, and remaining IOCs log `skip reason=deadline`; `whisper: error class=transport` line (auth ≠ transport ≠ query); no enrichment alert; no integratord hang. Measured live (2026-09-09, two-IOC alert, README defaults): blackholed API 22.0 s, 429 + `Retry-After: 60` 17.5 s |
...
| API budget & backoff | One wall-clock `deadline` per invocation (default 20 s) bounds everything: argv `timeout`(10 s)/`retries`(3) apply per request but shrink to what is left of the budget, a retry never sleeps past it, and a SIGALRM hard ceiling at deadline + 1 s covers a stalled socket or resolver. Backoff on 429/5xx honours `Retry-After` **up to a 5 s cap** (a synchronous integratord callout must never sleep for a minute); `max_iocs` (default 5) bounds enrichment work per alert; respect `retryAfter` from degraded `explain()`. Deadline/backoff arithmetic is verified in the **mocked unit tier** (tests/test_client.py, tests/test_deadline.py — no sleeps, no network) and the bound was measured live once (TC-13) | integratord runs integrations serially, one alert at a time — a slow API must cost seconds, not minutes (wazuh/integrations review, 2026-09) |
...
5. **`related.neighbors[]` (IP co-hosting) and `owner`/`business_unit` are deferred** — mapping
   §11 Q4/Q6.
6. **Five implementation knobs are required by this plan** (untestable without them):
   `dedup_ttl`, `api_url`, `deadline` and `max_iocs` configurable via the `<options>` JSON
   (argv[5]) → env → default (TC-10, TC-13), and a **flushable** dedup cache at
   `/var/ossec/var/whisper/dedup.db` (TC-01/09/12/17). Normative in mapping §2.3/§7.5.
